...
class DB {
	...
	public static function getXenoDB() {
		...
		foreach ( $variables_to_sync as $variable ) {
			if ( $xdb->$variable !== $db_class_vars[$variable] ) {
				$xdb->$variable = $db_class_vars[$variable];
			}
		}
...
class XenoDB {
	...
	public function nonSQLError( $message ) {
		if ( $this->throw_exception_on_nonsql_error ) {
			$e = new XenoDBException( $message );
			throw $e;
		}
		
		$error_handler = is_callable( $this->nonsql_error_handler ) ? $this->nonsql_error_handler : 'Xenodb_error_handler';
	...
	protected function preparseQueryParams() {
		...
		//	generate list of all XenoDB variables in our query, and their position
		...
		//	for each XenoDB variable, substitute it with array( type: i, value: 53 ) or whatever
	...
	protected function sanitize( $value ) {
		if ( is_object( $value ) ) {
			if ( $value instanceof XenoDBEval ) return $value->text;
	...
	protected function parseQueryParams() {
		...
		foreach ( $chunkyQuery as $chunk ) {
			...
			else $this->nonSQLError( "Badly formatted SQL query: Invalid XenoDB param $type" );
	...
	protected function queryHelper() {
		...
		if ( !$sql || $db->error ) {
			if ( $this->error_handler ) {
				$error_handler = is_callable( $this->error_handler ) ? $this->error_handler : 'Xenodb_error_handler';
			...
			if ( $this->throw_exception_on_error ) {
				$e = new XenoDBException( $db->error, $sql, $db->errno );
				throw $e;
			}
		} else if ( $this->success_handler ) {
			$runtime = sprintf( '%f', $runtime * 1000 );
			$success_handler = is_callable( $this->success_handler ) ? $this->success_handler : 'Xenodb_debugmode_handler';
...
class XenoDBException extends Exception {
	protected $query = '';
...
function Xenodb_error_handler( $params ) {
...
function Xenodb_debugmode_handler( $params ) {
